...
enum StreamingMode { EXPLORATION, COMBAT }
...
func _ready() -> void:
	GameAutoLoad.chunk_manager = self
...
func _update_room_streaming(current_room_id: String) -> void:
	var rooms_to_load: Array[String] = _get_rooms_in_radius(current_room_id, _get_radius())
...
func _load_room(id: String) -> void:
	...
	var grid_pos := Vector2i(
		roundi(pos_v3.x / room_scene_size), roundi(depth_axis / room_scene_size)
	)
