...
func _handle_projectile_hit(projectile: Projectile, collider: Node, hit_pos: Vector3, hit_norm: Vector3) -> bool:
...
func _update_projectile(projectile: Projectile, delta: float) -> void:
	...
	if not hit.is_empty():
		...
		if collider.has_method("apply_hit") or (collider.get_parent() and collider.get_parent().has_method("apply_hit")):
