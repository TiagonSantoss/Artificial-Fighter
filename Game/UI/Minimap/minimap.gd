...
func _draw() -> void:
	...
	var current_center_v3: Vector3 = layout.get(
		current_room_id,
		Vector3.ZERO
	)
	...
	for id in graph.connections.keys():
		...
		for conn in graph.connections[id]:
			...
			and not _is_room_visible(conn.target_id):
				continue
			
			var pos_a := _get_screen_pos(
				layout[id],
				current_center_v3,
				ui_center,
				cam_angle
	...
	for id in layout.keys():
		...
		var room_center := _get_screen_pos(
			layout[id],
			current_center_v3,
			ui_center,
			cam_angle
		...
		var outline_rect := box_rect.grow(
			outline_thickness
		)
