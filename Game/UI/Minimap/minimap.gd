...
func _draw() -> void:
	...
	# --- FIXED CAMERA ANGLE FETCHING ---
	var cam_angle := 0.0
	if Game.instance:
		# Use the smooth angle variable actively calculated by Game.gd's process loop
		# We invert it (negative) so the layout correctly rotates opposite to your camera perspective view
		cam_angle = Game.instance.camera_rig.rotation.y
	# ------------------------------------
	
	# STEP 1: DRAW CONNECTOR LINES
	for id in graph.connections.keys():
		...
		for conn in graph.connections[id]:
			...
			if _is_room_visible(id) or _is_room_visible(conn.target_id):
				...
				var pos_b := _get_screen_pos(layout[conn.target_id], current_center_v3, ui_center, cam_angle)
...
func _get_screen_pos(room_pos_v3: Vector3, current_center_v3: Vector3, ui_center: Vector2, cam_angle: float) -> Vector2:
	var relative := room_pos_v3 - current_center_v3
	
	var pos := Vector2(relative.x, relative.z) * minimap_scale
	
	# rotate around origin (center of minimap)
	var rotated := Vector2(
